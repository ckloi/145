...
secretencoder <- function(imgfilename,msg,startpix,stride,consec = NULL){
  ...
  # This will loop through msg one element (character) at a time, and will place
  # the appropriate character at every stride pixels in the image.
  pa[seq(1,length(msg)*stride,stride)] <- utf8ToInt(msg[seq(1,length(msg),1)]) / 128
