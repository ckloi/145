        ...
        for (i in values){
            while(1){
                ...
                if(!(index %in% pixAddress)){
                    sumUpDown <- checkNeighbors(index,pixAddress,pa, consec, 1) +  checkNeighbors(index,pixAddress,pa, consec,2)
                    # If column has no conflict, check row
                    if(sumUpDown <= consec-1){
                        sumLeftRight <- checkNeighbors(index,pixAddress,pa, consec,3) + checkNeighbors(index,pixAddress,pa,consec,4)
...
checkNeighbors <- function(index,array,pa,consec,mode){
